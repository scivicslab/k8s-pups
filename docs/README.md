...
- 開発者端末（10.0.0.14）からは Green（set2）経由: `https://10.0.0.25/pups/`
- その他からは Blue（set1）経由: `https://10.0.0.25/pups/`
...
```
src/main/java/com/github/user1/quarkus/scaccount/e2e/helper/KubectlExec.java
```
...
```
src/main/java/com/github/user1/quarkus/scaccount/e2e/base/E2EConfig.java
```
...
```
src/main/java/com/github/user1/quarkus/scaccount/e2e/helper/PostgresHelper.java
```
...
// 暗号化パスワード取得
String cipher = PostgresHelper.getEncryptedPassword("admin");
```

#### `LdapHelper.java` / `LdapEntry.java`

```
src/main/java/com/github/user1/quarkus/scaccount/e2e/helper/LdapHelper.java
src/main/java/com/github/user1/quarkus/scaccount/e2e/helper/LdapEntry.java
```
...
```java
// ユーザー検索
LdapEntry entry = LdapHelper.findUser("admin");
...
kubectl exec -n sc-account-bg test-pod -- \
  ldapsearch -H ldap://ldap-set2:389 -x -b "dc=example,dc=com" "(uid=admin)" uid
```
...
sudo docker tag k8s-pups:$TAG 10.0.0.23:32000/k8s-pups:$TAG
sudo docker push 10.0.0.23:32000/k8s-pups:$TAG

# Deploymentのimageタグを更新して適用
kubectl set image deployment/k8s-pups-controller -n k8s-pups k8s-pups=10.0.0.23:32000/k8s-pups:$TAG
...
# ヘルスチェック（VIP経由）
curl -sk https://10.0.0.25/pups/q/health | jq .status

# ダッシュボード（ブラウザでアクセスするとKeycloakリダイレクト）
curl -sk https://10.0.0.25/pups/dashboard -o /dev/null -w "%{http_code}\n"
# → 302（Keycloakへリダイレクト）
```
