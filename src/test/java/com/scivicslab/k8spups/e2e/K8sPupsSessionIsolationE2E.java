 ...
 *
 * Flow:
 *   1. admin logs in and starts a file-browser session.
 *   2. Verify admin can access the session URL (sanity check).
 *   3. testuser logs in (separate browser context).
 *   4. testuser attempts to access admin's session URL.
...
class K8sPupsSessionIsolationE2E extends K8sPupsE2EBase {
    ...
    private void verifySessionIsolation() {
        LOG.info("=== Session isolation: admin's session is inaccessible to testuser ===");

        // --- Step 1: admin launches a file-browser session ---
        login();
        ...
        LOG.info("admin session URL: " + sessionUrl);

        // --- Step 2: verify admin CAN access the session (sanity check) ---
        ...
        LOG.info("admin → session URL: HTTP " + adminStatus + " final=" + page.url());

        if (adminStatus == 404) {
            throw new AssertionError(
                    "Session routing is broken: admin gets HTTP 404 at " + sessionUrl
                    + " — cannot test isolation against a non-existent route");
        }
        if (adminStatus == 403) {
            throw new AssertionError(
                    "Session routing is broken: admin gets HTTP 403 for their own session at " + sessionUrl);
        }
        LOG.info("Sanity check PASSED: admin can reach session (HTTP " + adminStatus + ")");
        ...
        // --- Step 4: testuser attempts to access admin's session URL ---
        LOG.info("testuser attempting to access admin's session: " + sessionUrl);
        ...
        LOG.info("Session isolation PASSED: testuser received HTTP " + userStatus
                + " — cannot access admin's session");

        // --- Cleanup: stop admin's session ---
        stopExistingSessionIfAny("file-browser");
    }
}
